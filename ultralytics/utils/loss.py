...
class v8DetectionLoss:
    ...
    def __call__(self, preds, batch, epoch=1):
    ...
    def __call__(self, preds, batch, epoch=1):
        ...
        if fg_mask.sum():
            ...
            loss[0], loss[4] = self.bbox_loss(
                pred_distri, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask, epoch=1
            )
...
class E2EDetectLoss:
    ...
    def __call__(self, preds, batch, epoch=1):
        ...
        loss_one2many = self.one2many(one2many, batch, epoch)
        one2one = preds["one2one"]
        loss_one2one = self.one2one(one2one, batch, epoch)
