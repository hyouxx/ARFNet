...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (Detect, ASFFHead, AFPNHead, AFPN4Head, DynamicHead, DynamicDCNv3Head, DSDConvHead, FASFFHead, FADDWConvHead)):  # includes all Detect subclasses like Segment, Pose, OBB, WorldDetect
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolov8n.yaml", ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        # Build strides
        m = self.model[-1]  # Detect()
        if isinstance(m, (Detect, ASFFHead, FASFFHead, DynamicHead, AFPNHead, AFPN4Head, DynamicDCNv3Head, DSDConvHead, DSDConvSegment, FADDWConvHead)):  # includes all Detect subclasses like Segment, Pose, OBB, WorldDetect
            s = 640  # 2x min stride
            ...
            def _forward(x):
                ...
                return self.forward(x)[0] if isinstance(m, (Segment, Pose, OBB, DSDConvSegment)) else self.forward(x)
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            Focus, BottleneckCSP, C1, C2, C2f, C3k2, C2PSA_iRMB,
        }:
            c1, c2 = ch[f], args[0]
...
def guess_model_task(model):
    ...
    if isinstance(model, nn.Module):  # PyTorch modeldsdconvsegment
        ...
        for m in model.modules():
            if isinstance(m, (Segment, DSDConvSegment)):
                return "segment"
                ...
                return "obb"
            elif isinstance(m, (Detect, WorldDetect, v10Detect, ASFFHead, AFPNHead, AFPN4Head, DynamicHead, DynamicDCNv3Head,
                                DSDConvHead, FASFFHead, FADDWConvHead)):
                return "detect"
